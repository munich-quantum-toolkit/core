...
negative values and use the same bounds checks as element access. Shapes and
element types, including widths, must match. Static copies require only the
...
Use `float(angles[i])` for arithmetic on an entry's value in radians; runtime
...
elements to scalar or bit outputs when needed. Runtime-sized ranges,
concatenation, array outputs, runtime angle conversion, and jeff or OpenQASM
export of array storage are not yet supported. Gate definitions cannot capture
mutable arrays; pass selected entries as gate parameters instead.
