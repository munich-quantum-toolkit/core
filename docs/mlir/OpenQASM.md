...
| Dynamic qubit and bit indexing | Supported | Structured dispatch is bounded to 4096 leaves; target restrictions are listed below | `DispatchesDynamicQubitGatesWithStructuredControlFlow` |
...
| Structured dynamic dispatch budget | Supported | Rejected above 4096 leaves | Not reached | Not reached | Not reached | Semantic analysis reports the source location before exponential dispatch construction | `RejectsExcessiveDynamicDispatch` |
| Custom-gate expansion budget | Supported | Supported | Rejected above 100000 primitive applications | Rejected | Rejected | QC preflight bounds recursive inline expansion before constructing IR | `RejectsExcessiveCustomGateExpansion` |
...
Features accepted by QC emission are required to pass the demonstrated compiler
path. Features that cannot retain their semantics through that path fail with a
