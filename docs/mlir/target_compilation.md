...
control flow, and qubit tensors or vectors carried through generic structured
control remain unsupported. A {code}`qco.if` may carry statically shaped,
rank-one qubit tensors only when cleanup can scalarize every constant-index
branch access; dynamic, partial, or repeated tensor access remains unsupported.
The preflight rejects unsupported forms before mapping even when every listed
