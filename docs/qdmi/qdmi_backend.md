...
`seed_simulator`. Provider-specific backends can define custom seed parameters.
Other execution options are unsupported. The backend validates the whole batch
before submission, submits jobs in circuit order, and collects results in that
order. Remote IDs are queried only when needed. Submission or collection failure
triggers best-effort cancellation of submitted jobs; cancellation errors do not
replace the original error. Missing memory, invalid bitstrings or shot totals,
and failed or canceled jobs raise instead of yielding partial or zero-filled
samples. Successful repeated reads reuse the result.
