...
for (const auto& definition : manager.definitions()) {
  try {
    ...
    // An unavailable device implementation does not affect other definitions.
  }
}
```
...
selected definition, and separate definitions can share one loaded device
...
device implementation to abort the remaining opens.
