...
A license name selects a stable device ID and nothing else, so a cluster needs
one registered device ID for every resource that its jobs contend for.

One provider library can serve several quantum computers, so register a device
definition for each machine. The definitions share a `library` and a `prefix`
and differ only in the `session` object that selects the machine, and MQT Core
initializes the library once and then opens one session per definition. The MQT
Core superconducting provider works this way: `mqt.sc.iqm.garnet` and
`mqt.sc.iqm.emerald` are separate stable IDs over the library that also provides
`mqt.sc.default`.
...
Each machine then gets its own license, which Slurm regulates separately:
...
definitions with the `DEVICES` argument of `mqt_configure_qdmi_device`, as
described in [Device configuration](configuration.md).

Do not license a device that has no target until a job supplies one. A provider
registers such a device so that a caller can choose any machine at run time, and
a job that requests it has named no machine, so there is no contention for Slurm
to regulate. A device that always resolves to the same resource, such as
`mqt.sc.default`, is itself a contended resource and can carry a license.
...
match exactly. The error message lists every registered ID, so compare that list
against the license name. If a provider serves more machines than it registers
IDs, see [](#license-one-device-id-per-contended-resource). Do not add a generic
device license. Do not use a Slurm OR license expression for device selection
because the environment does not identify a single selected device in that case.
