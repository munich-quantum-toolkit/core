...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/core/issues>. Include steps to
...
- 💡 Propose features:
  ...
  <https://github.com/munich-quantum-toolkit/core/issues>. Describe the
...
{code}`-DBUILD_MQT_CORE_TESTS=OFF` to the CMake configure step.
:::

Our CI pipeline on GitHub also collects code coverage information and uploads it
to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/core). Our goal is to
...
To build only the Python bindings, pass {code}`-DBUILD_MQT_CORE_BINDINGS=ON` to
...
in the {code}`python/mqt/core` directory. These stub files are auto-generated.
...
session fail and must not be treated as a support declaration.
...
MQT Core adheres to [Semantic Versioning], with the exception that minor
...
a breaking change for MQT Core. If you are sure that the dependency update does
not introduce any breaking changes for MQT Core, you can remove the
...
[Releases page](https://github.com/munich-quantum-toolkit/core/releases) on
