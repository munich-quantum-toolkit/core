...
We use GitHub to [host code](https://github.com/munich-quantum-toolkit/core), to [track issues and feature requests][issues], as well as accept [pull requests](https://github.com/munich-quantum-toolkit/core/pulls).
...
- 🐛 Report bugs:
  ...
  Use the _🐛 Bug report_ template at <https://github.com/munich-quantum-toolkit/core/issues>.
...
- 💡 Propose features:
  ...
  Use the _✨ Feature request_ template at <https://github.com/munich-quantum-toolkit/core/issues>.
...
- 🙌 Community support:
  ...
  <https://github.com/munich-quantum-toolkit/core/discussions>.
...
:::{tip}
If you want to disable configuring and building the C++ tests, you can pass {code}`-DBUILD_MQT_CORE_TESTS=OFF` to the CMake configure step.
:::
...
To build only the Python bindings, pass {code}`-DBUILD_MQT_CORE_BINDINGS=ON` to the CMake configure step.
...
For the compiled parts of the code base, we provide type hints in the form of stub files in the {code}`python/mqt/core` directory.
...
When it is time to release a new version of MQT Core, create a PR that prepares the release.
This PR should:
...
This does not mean that the dependency update itself is a breaking change for MQT Core.
If you are sure that the dependency update does not introduce any breaking changes for MQT Core, you can remove the {code}`minor` or {code}`major` label from the PR.
...
Afterward, navigate to the [Releases page](https://github.com/munich-quantum-toolkit/core/releases) on GitHub, edit the release draft if necessary, and publish the release.
