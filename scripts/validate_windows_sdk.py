...
def main() -> None:
    ...
    execute(
        ...
        [
            ...
            "-Ccmake.define.BUILD_MQT_CORE_TESTS=ON",
            "-Ccmake.define.LLVM_DIR=" + str(sdk / "lib/cmake/llvm"),
            "-Ccmake.define.MLIR_DIR=" + str(sdk / "lib/cmake/mlir"),
        ],
    )
    execute("cpp-build", ["cmake", "--build", str(build), "--config", "Release", "-j", "4"])
    execute("cpp-tests", ["ctest", "--test-dir", str(build), "-C", "Release", "--output-on-failure", "-j", "4"])
