...
auto Edge<Node>::getValueByPath(const std::size_t numQubits,
    ...
    -> Result<std::complex<fp>> {
  if (decisions.size() < numQubits) {
    return Error{
        .message = "Decision path is shorter than the number of qubits.",
  ...
  if (path.find_first_not_of(IsVector<Node> ? "01" : "0123") !=
    ...
    return Error{.message = "Decision path contains an invalid digit."};
  }
  ...
  requires IsVector<Node>
{
  ...
  if (numQubits < std::numeric_limits<size_t>::digits &&
      (i >> numQubits) != 0U) {
    return Error{
        .message = "Vector index is out of range.",
        .kind = Error::Kind::OutOfRange,
    };
  }
  auto edge = *this;
  ...
  requires IsVector<Node>
{
  ...
  for (auto i = 0ULL; i < element; i++) {
    const auto amplitude = std::get<0>(getValueByIndex(i));
  ...
  requires IsMatrix<Node>
{
  if (numQubits < std::numeric_limits<size_t>::digits &&
    ...
    return Error{
        .message = "Matrix index is out of range.",
        .kind = Error::Kind::OutOfRange,
    };
  }
  if (isTerminal()) {
  ...
  requires IsMatrix<Node>
{
  ...
  for (auto i = 0ULL; i < element; ++i) {
    for (auto j = 0ULL; j < element; ++j) {
      const auto amplitude = std::get<0>(getValueByIndex(numQubits, i, j));
      std::cout << amplitude << " ";
    }
    std::cout << "\n";
  }
