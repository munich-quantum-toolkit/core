...
mlir::FailureOr<QFTAdder> QFTAdder::create(QFTAdderOptions options) {
  ...
  if (options.overflow != QFTAdderOverflow::Wrap &&
    ...
    return ::mqt::emitError("QFT adder overflow must be wrap or carry",
                            ::mqt::ErrorCategory::InvalidArgument);
  }
  ...
  if (width == 0 ||
    ...
    return ::mqt::emitError("QFT adder operands must have equal nonzero "
  ...
  if (options.addend.find_first_not_of(isRegister ? "01+" : "01") !=
    ...
    return ::mqt::emitError("QFT adder operands must be binary; only "
