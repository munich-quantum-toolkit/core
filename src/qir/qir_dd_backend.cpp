...
void __quantum__qis__u1__body(const double phi, Qubit* qubit) {
  auto& backend = mqt::QIR_DD_Backend::getInstance();
  backend.apply(qc::P, phi, qubit);
}

void __quantum__qis__cu1__body(const double phi, Qubit* control,
                               Qubit* target) {
  auto& backend = mqt::QIR_DD_Backend::getInstance();
  backend.apply(qc::P, phi, control, target);
}

void __quantum__qis__cu3__body(const double theta, const double phi,
                               const double lambda, Qubit* control,
                               Qubit* target) {
  auto& backend = mqt::QIR_DD_Backend::getInstance();
...
void __quantum__qis__crx__body(const double phi, Qubit* control,
                               Qubit* target) {
  auto& backend = mqt::QIR_DD_Backend::getInstance();
