...
using qasm3::BitIndexToRegisterMap;
using qasm3::QubitIndexToRegisterMap;
...
void dumpOperation(std::ostream& output, const Operation& operation,
                   ...
                   bool openQASM3,
                   const qasm3::Serializer::CustomOperationSerializer&
...
void Serializer::serialize(const qc::QuantumComputation& computation) const {
  const auto openQASM3 = format == qc::Format::OpenQASM3;

  qc::Permutation qubitToIndex{};
  ...
  qc::Permutation inverseOutputPermutation{};
  ...
  qc::printSortedRegisters(combinedRegs, openQASM3 ? "qubit" : "qreg", output,
                           openQASM3);

  const auto& classicalRegisters = computation.getClassicalRegisters();
  qc::printSortedRegisters(classicalRegisters, openQASM3 ? "bit" : "creg",
                           output, openQASM3);
...
void Serializer::serialize(const qc::Operation& operation,
                           const QubitIndexToRegisterMap& qubitMap,
                           const BitIndexToRegisterMap& bitMap,
                           const std::size_t indent) const {
  qc::dumpOperation(output, operation, qubitMap, bitMap, indent,
                    format == qc::Format::OpenQASM3, customOperationSerializer);
}
