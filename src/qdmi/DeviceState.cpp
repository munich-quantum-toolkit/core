...
openSession(const DeviceApi& api, const DeviceSessionConfig& parameters,
  ...
  try {
    ...
    if (parameters.deviceConfiguration &&
        (parameters.custom1 || parameters.custom2)) {
    ...
    if (parameters.deviceConfiguration) {
      std::visit(
          [&set](const auto& source) {
            using Source = std::decay_t<decltype(source)>;
            if constexpr (std::same_as<Source, InlineDeviceConfiguration>) {
              set(source.json, QDMI_DEVICE_SESSION_PARAMETER_CUSTOM1);
            } else {
              set(source.path.string(), QDMI_DEVICE_SESSION_PARAMETER_CUSTOM2);
            }
          },
...
DeviceState::DeviceState(std::shared_ptr<const DeviceApi> deviceApi,
                         ...
                         QDMI_Child_Device child,
                         std::shared_ptr<SessionState> parentSession)
    : sessionState(std::make_shared<SessionState>(std::move(deviceApi),
                                                  sessionParameters, child,
  ...
  for (auto* handle : handles) {
    children.emplace_back(std::make_shared<DeviceState>(
        sessionState->api, parameters, handle, sessionState));
  }
}
