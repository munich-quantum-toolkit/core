...
struct DeviceApi {
  ...
  decltype(QDMI_device_session_retrieve_device_job_by_id)*
      device_session_retrieve_device_job_by_id = nullptr;
...
///
/// The cache is weak: live sessions share one initialized library keyed by its
/// canonical path and prefix, and the library unloads after the last session is
/// destroyed. A replacement generation waits until prior finalization and
/// unloading have completed.
