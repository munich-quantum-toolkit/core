...
TEST_F(WireIteratorFixture, UnitaryCallContinuesWire) {
  QCOProgramBuilder builder(context.get());
  builder.initialize();
  ...
  ++iterator;
  EXPECT_TRUE(isa<CallOp>(iterator.operation()));
  EXPECT_EQ(iterator.qubit(), output);
  --iterator;
  EXPECT_EQ(iterator.qubit(), input);
}

TEST_F(WireIteratorFixture, GenericCallIsWireBoundary) {
  QCOProgramBuilder builder(context.get());
  builder.initialize();
  auto reset = builder.createFunction(
      "reset", TypeRange{QubitType::get(context.get())},
      [&](ValueRange arguments) {
  ...
  ++forward;
  EXPECT_TRUE(isa<func::CallOp>(forward.operation()));
  ++forward;
  EXPECT_EQ(forward, std::default_sentinel);

  WireIterator backward(output);
  --backward;
  EXPECT_EQ(backward, std::default_sentinel);
}
