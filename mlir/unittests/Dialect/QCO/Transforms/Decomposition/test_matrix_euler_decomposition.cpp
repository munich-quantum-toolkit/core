...
class EulerDecompositionTest
    : public testing::TestWithParam<
          std::tuple<GateEulerBasis, Eigen::Matrix2cd (*)()>> {
public:
...
TEST(EulerDecompositionTest, Random) {
  ...
  auto eulerBases = std::array{GateEulerBasis::XYX, GateEulerBasis::XZX,
...
TEST(EulerDecompositionTest, ZyzAnglesFromUnitaryReconstructHadamard) {
  ...
  const auto angles =
      EulerDecomposition::anglesFromUnitary(hadamard, GateEulerBasis::ZYZ);
...
TEST(EulerDecompositionTest, NativeEulerBasesRandomReconstruction) {
  ...
  for (int i = 0; i < 24; ++i) {
    ...
    const auto u3Seq = EulerDecomposition::generateCircuit(
        GateEulerBasis::U3, unitary, true, std::nullopt);
    const auto zsxSeq = EulerDecomposition::generateCircuit(
        GateEulerBasis::ZSX, unitary, true, std::nullopt);
    const auto zsxxSeq = EulerDecomposition::generateCircuit(
        GateEulerBasis::ZSXX, unitary, true, std::nullopt);
...
TEST(EulerDecompositionTest, ZsxxPauliXUsesSingleXGate) {
  ...
  const auto seq = EulerDecomposition::generateCircuit(
      GateEulerBasis::ZSXX, pauliX, true, std::nullopt);
...
TEST(EulerDecompositionTest, GetGateTypesForEulerBasis) {
  const auto zyz = getGateTypesForEulerBasis(GateEulerBasis::ZYZ);
  ASSERT_EQ(zyz.size(), 2U);
  EXPECT_EQ(zyz[0], GateKind::RZ);
  EXPECT_EQ(zyz[1], GateKind::RY);

  const auto uFamily = getGateTypesForEulerBasis(GateEulerBasis::U321);
  ASSERT_EQ(uFamily.size(), 1U);
  EXPECT_EQ(uFamily[0], GateKind::U);

  const auto zsxx = getGateTypesForEulerBasis(GateEulerBasis::ZSXX);
...
TEST(EulerDecompositionTest, UAndU321MatchU3Reconstruction) {
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    const auto seqU3 = EulerDecomposition::generateCircuit(
        GateEulerBasis::U3, u, true, std::nullopt);
    const auto seqU = EulerDecomposition::generateCircuit(GateEulerBasis::U, u,
                                                          true, std::nullopt);
    const auto seqU321 = EulerDecomposition::generateCircuit(
        GateEulerBasis::U321, u, true, std::nullopt);
...
TEST(EulerDecompositionTest, AnglesFromUnitaryXZXReconstructsRx) {
  ...
  (void)EulerDecomposition::anglesFromUnitary(u, GateEulerBasis::XZX);
  const auto seq = EulerDecomposition::generateCircuit(GateEulerBasis::XZX, u,
                                                       false, std::nullopt);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::Values(GateEulerBasis::XYX, GateEulerBasis::XZX,
                                     GateEulerBasis::ZYZ, GateEulerBasis::ZXZ),
                     testing::Values(
                         []() -> Eigen::Matrix2cd {
