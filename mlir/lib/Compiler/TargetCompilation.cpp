...
[[nodiscard]] static bool hasUnsupportedQubitTensorState(Operation* operation) {
  if (llvm::isa<qco::IfOp>(operation)) {
    const auto isUnsupportedTensor = [](const Type type) {
      const auto tensor = llvm::dyn_cast<RankedTensorType>(type);
      return tensor && llvm::isa<qco::QubitType>(tensor.getElementType()) &&
             (tensor.getRank() != 1 || !tensor.hasStaticShape());
    };
    return llvm::any_of(operation->getOperandTypes(), isUnsupportedTensor) ||
           llvm::any_of(operation->getResultTypes(), isUnsupportedTensor);
  }
...
private:
  ...
  [[nodiscard]] LogicalResult verifyOperation(Operation* operation) const {
    ...
    if (const auto required = requiredClassicalControl(operation)) {
      ...
      const auto qcoIf = llvm::dyn_cast<qco::IfOp>(operation);
      if ((qcoIf != nullptr && !qco::hasOnlyScalarizableTensorInputs(qcoIf)) ||
          hasUnsupportedQubitTensorState(operation)) {
        operation->emitError()
