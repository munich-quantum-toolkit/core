...
#undef MQT_ADD_QC_TO_QCO_GATE
    ...
    terminatorTarget.addDynamicallyLegalOp<scf::YieldOp, scf::ConditionOp>(
        [&](Operation* op) {
          ...
          const auto expectedTypes = parentOp->getResultTypes();
          if (auto condition = dyn_cast<scf::ConditionOp>(op)) {
            return llvm::equal(condition.getArgs().getTypes(), expectedTypes);
          }
          return llvm::equal(op->getOperandTypes(), expectedTypes);
        });
