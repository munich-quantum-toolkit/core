...
std::size_t QubitGateSequence::complexity() const {
  ...
  if (hasGlobalPhase()) {
    // Count the same heuristic cost as an explicit global-phase gate.
    c += helpers::getComplexity(GateKind::GPhase, 0);
  }
  return c;
}
...
Eigen::Matrix4cd QubitGateSequence::getUnitaryMatrix() const {
  ...
  for (auto&& gate : gates) {
    // Left-multiply each gate matrix so the stored order matches execution
    // order in the reconstructed unitary.
