...
///
/// Qubits are threaded through the body, so a qubit that the body only
...
/// Rebuild a modifier with only the qubits used by its body.
///
...
/// Inline @p body into the modifier currently being built, dropping the
/// qubits it does not use, and return the qubits it yields for the others.
///
/// The block arguments of unused qubits are only yielded, and those
...
/// Replace used positions in @p inputs with @p narrowedResults.
[[nodiscard]] SmallVector<Value>
