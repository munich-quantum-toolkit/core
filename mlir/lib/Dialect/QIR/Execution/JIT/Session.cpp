...
mlir::FailureOr<int64_t> JitSession::sample(size_t shots,
  ...
  if (mlir::failed(runtime_->outputProgramHeader())) {
    runtime_->reset();
    return mlir::failure();
  }
  const auto execute = [&]() -> mlir::FailureOr<int64_t> {
    ...
    if (mlir::failed(runtime_->outputShotStart())) {
      runtime_->reset();
      return mlir::failure();
    }
    const auto code = run();
    if (mlir::failed(runtime_->outputShotEnd(code))) {
      runtime_->reset();
      return mlir::failure();
    }
    return code;
  ...
  if (samplingOutputs_ && !runtime_->hasOutput() && shots != 0) {
    ...
    const llvm::scope_exit restore(
        [&] { runtime_->deferMeasurements_ = false; });
    auto code = execute();
    if (mlir::failed(code)) {
      return mlir::failure();
    }
