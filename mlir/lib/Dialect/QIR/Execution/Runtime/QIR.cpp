...
static auto controlsFromArray(Array* array)
    -> llvm::Expected<llvm::SmallVector<Qubit*, 4>> {
  if (array == nullptr) {
    return llvm::createStringError(
        std::make_error_code(std::errc::invalid_argument),
        "QIR control array must not be null");
  }
...
static auto applyGateMatrix(llvm::ArrayRef<double> parameters,
  ...
  if constexpr (std::is_same_v<GateOp, mlir::qco::SWAPOp>) {
    if (controls.empty() && targets.size() == 2) {
      runtime.recordError(runtime.swap(targets[0], targets[1]));
      return;
    }
  }
  auto matrix = mlir::qco::getStandardGateMatrix<GateOp>(parameters);
  if (!matrix) {
    runtime.recordError(matrix.takeError());
    return;
  }
  runtime.recordError(runtime.apply(*matrix, controls, targets));
}
...
static auto applyControlled(Array* controlArray, Qubit* target) -> void {
  auto controls = controlsFromArray(controlArray);
  if (!controls) {
    qir::Runtime::getInstance().recordError(controls.takeError());
    return;
  }
...
static auto applyControlledTuple(Array* controls, Tuple* tuple) -> void {
  auto& runtime = qir::Runtime::getInstance();
  if (tuple == nullptr) {
    runtime.recordError(llvm::createStringError(
        "QIR generic controlled argument tuple must not be null"));
    return;
  }
  const auto apply = [&](auto& args, llvm::ArrayRef<double> parameters) {
    ...
    auto controlList = controlsFromArray(controls);
    if (!controlList) {
      runtime.recordError(controlList.takeError());
      return;
    }
...
/// Allocation errors reported through outError remain recoverable by QIR.
/// Other errors stop instrumented generated code at its next call boundary.
template <typename T>
static T abiResult(llvm::Expected<T> result, bool* outError = nullptr) {
  if (outError != nullptr) {
    *outError = !result;
  }
  if (result) {
  ...
  if (outError != nullptr) {
    ...
    qir::Runtime::getInstance().recordError(result.takeError());
  }
  return T{};
}
  ...
  if (size < 0 || (size > 0 && array == nullptr)) {
    if (outError != nullptr) {
      *outError = true;
      ...
      runtime.recordError(
          llvm::createStringError("Invalid QIR resource array allocation"));
    }
    return;
  }
  for (int64_t i = 0; i < size; ++i) {
    ...
    if (!result) {
      for (int64_t j = 0; j < i; ++j) {
        if constexpr (std::is_same_v<T, Qubit>) {
          llvm::cantFail(runtime.qFree(array[j]));
        } else {
          llvm::cantFail(runtime.rFree(array[j]));
        }
        array[j] = nullptr;
      }
...
template <typename T> static void releaseArray(int64_t size, T** array) {
  ...
  if (size < 0 || (size > 0 && array == nullptr)) {
    runtime.recordError(
        llvm::createStringError("Invalid QIR resource array release"));
    return;
  }
  for (auto* value : std::span(array, static_cast<size_t>(size))) {
    auto error = [&] {
      if constexpr (std::is_same_v<T, Qubit>) {
        return runtime.qFree(value);
      } else {
        return runtime.rFree(value);
      }
    }();
    if (error) {
      runtime.recordError(std::move(error));
      return;
    }
  }
}
...
bool __mqt__qir__has_error() noexcept {
  return qir::Runtime::getInstance().hasError();
}

// *** ARRAYS ***
Array* __quantum__rt__array_create_1d(const int32_t size,
                                      const int64_t n) noexcept {
  return abiResult([&]() -> llvm::Expected<Array*> {
    if (size <= 0 || n < 0) {
      return llvm::createStringError(
          std::make_error_code(std::errc::invalid_argument),
    ...
    if (length > maxObjectSize / elementSize) {
      return llvm::createStringError("QIR array allocation size overflow");
    }
    auto array = std::make_unique<Array>();
    array->refcount = 1;
    array->data = std::vector(length * elementSize, static_cast<int8_t>(0));
    array->elementSize = size;
    qir::Runtime::getInstance().ownAllocation(array.get(), [](void* pointer) {
      delete static_cast<Array*>(pointer);
    });
...
int64_t __quantum__rt__array_get_size_1d(const Array* array) noexcept {
  if (array == nullptr) {
    qir::Runtime::getInstance().recordError(
        llvm::createStringError("QIR array must not be null"));
    return 0;
  }
...
int8_t* __quantum__rt__array_get_element_ptr_1d(Array* array,
  ...
  if (array == nullptr || i < 0 ||
    ...
    qir::Runtime::getInstance().recordError(
        llvm::createStringError("QIR array index is out of range"));
    return nullptr;
  }
...
Tuple* __quantum__rt__tuple_create(const int64_t size) noexcept {
  return abiResult([&]() -> llvm::Expected<Tuple*> {
    if (size < 0) {
      return llvm::createStringError(
          std::make_error_code(std::errc::invalid_argument),
          "QIR tuple size must not be negative");
    }
    const auto payloadSize = static_cast<std::size_t>(size);
    constexpr auto maxObjectSize =
        static_cast<std::size_t>(std::numeric_limits<std::ptrdiff_t>::max());
    if (payloadSize > maxObjectSize - sizeof(TupleHeader)) {
      return llvm::createStringError("QIR tuple allocation size overflow");
    }
...
void __quantum__rt__result_release(Result* result) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  runtime.recordError(runtime.rFree(result));
}
...
void __quantum__rt__qubit_release(Qubit* qubit) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  runtime.recordError(runtime.qFree(qubit));
}
...
void __quantum__qis__gphase__body(const double phase) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  runtime.recordError(runtime.applyGlobalPhase(phase));
}
...
void __quantum__qis__mz__body(Qubit* qubit, Result* result) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  runtime.recordError(runtime.measure(qubit, result));
}

void __quantum__qis__reset__body(Qubit* qubit) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  runtime.recordError(runtime.reset(std::array{qubit}));
}
...
bool __quantum__rt__read_result(Result* result) noexcept {
  auto& runtime = qir::Runtime::getInstance();
  auto value = runtime.deref(result);
  if (!value) {
    runtime.recordError(value.takeError());
    return false;
  }
  return value->r;
}
...
void __quantum__rt__result_record_output(Result* result,
  ...
  qir::Runtime::getInstance().recordError(runtime.outputResult(bit, label));
...
void __quantum__rt__result_array_record_output(const int64_t size,
  ...
  if (size < 0 || (size > 0 && results == nullptr)) {
    qir::Runtime::getInstance().recordError(
        llvm::createStringError("Invalid QIR result array output"));
    return;
  }
  auto& runtime = qir::Runtime::getInstance();
  std::string values;
  ...
  for (Result* result : std::span(results, static_cast<std::size_t>(size))) {
    auto resolved = runtime.deref(result);
    if (!resolved) {
      runtime.recordError(resolved.takeError());
      return;
    }
    const auto value = resolved->r;
    if (runtime.hasOutput()) {
