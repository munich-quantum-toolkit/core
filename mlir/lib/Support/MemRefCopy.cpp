...
LogicalResult
LowerMemRefCopy::matchAndRewrite(memref::CopyOp copy,
  ...
  if (!sourceType || !targetType ||
      (onlyStrided_ && memref::isStaticShapeAndContiguousRowMajor(sourceType) &&
  ...
  scf::buildLoopNest(
      ...
      [&](OpBuilder& builder, Location location, ValueRange indices) {
        auto value = memref::LoadOp::create(builder, location, copy.getSource(),
                                            indices);
