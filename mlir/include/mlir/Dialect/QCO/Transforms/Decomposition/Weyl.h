...
/// Maps `|Tr(U^dag V)|` to average gate fidelity for 2-qubit unitaries.
///
...
/// Weyl decomposition of a 2-qubit unitary.
///
...
public:
  ...
  /// Single-qubit factor on qubit 0 after the canonical gate.
  [[nodiscard]] const Matrix2x2& k1l() const { return k1l_; }
  /// Single-qubit factor on qubit 0 before the canonical gate.
  [[nodiscard]] const Matrix2x2& k2l() const { return k2l_; }
  /// Single-qubit factor on qubit 1 after the canonical gate.
  [[nodiscard]] const Matrix2x2& k1r() const { return k1r_; }
  /// Single-qubit factor on qubit 1 before the canonical gate.
  [[nodiscard]] const Matrix2x2& k2r() const { return k2r_; }

  [[nodiscard]] static Matrix4x4 getCanonicalMatrix(double a, double b,
                                                    double c);

  /// Reconstructs `U = (K1l ⊗ K1r) · CAN(a,b,c) · (K2l ⊗ K2r) · e^{iφ}`.
...
/// Native two-qubit synthesis result.
///
...
/// Decomposer for a fixed two-qubit basis gate (e.g. CX/CZ).
///
...
public:
  /// Precomputes basis-gate data for repeated target decompositions.
  ///
...
/// Result of two-qubit synthesis, including its accumulated phase.
