...
def test_device_ids_and_bulk_open_result() -> None:
    ...
    manager = DeviceManager(DeviceRegistry([definition, bad]))
...
def test_device_definition_accepts_path_like_library() -> None:
    """Device definitions preserve filesystem paths without loading them."""
    library = Path("/nonexistent/lib.so")
    definition = DeviceDefinition("python.missing", library, "PREFIX")
    assert definition.library == library

    registry = DeviceRegistry([definition])
    with pytest.raises(RuntimeError):
        DeviceManager(registry).open(definition.device_id)


def test_session_parameters_support_keyword_construction() -> None:
    """Session parameters accept optional keywords and preserve path values."""
    auth_file = Path("/path/to/auth.json")
    credential = auth_file.name
    ...
    assert parameters.base_url == "https://example.com"
    assert parameters.token == credential
    assert parameters.auth_file == auth_file
    assert parameters.auth_url == "https://example.com/auth"
    assert parameters.username == "username"
    assert parameters.password == credential
    assert parameters.custom1 == "one"
    assert parameters.custom2 == "two"
    assert parameters.custom3 == "three"
    assert parameters.custom4 == "four"
    assert parameters.custom5 == "five"

    empty = SessionParameters()
    assert empty.base_url is None
    assert empty.auth_file is None
