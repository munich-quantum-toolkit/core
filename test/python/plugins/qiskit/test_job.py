...
def recording_backend(
    ...
    Returns:
        The backend, submitted jobs, and ordered events.
    """
...
def test_batch_order_and_repeated_reads(recording_backend: RecordingBackend, *, memory: bool) -> None:
...
def test_memory_order_registers_and_correlations(
...
def test_result_header_snapshot(recording_backend: RecordingBackend) -> None:
...
def test_malformed_memory_raises(recording_backend: RecordingBackend, samples: list[str]) -> None:
...
def test_malformed_histogram_raises(recording_backend: RecordingBackend, counts: dict[str, int | float]) -> None:
...
def test_invalid_options(recording_backend: RecordingBackend, options: dict[str, object]) -> None:
    ...
    with pytest.raises(CircuitValidationError):
        backend.run(QuantumCircuit(1, 1), parameter_values=None, **options)
    assert not jobs


def test_backend_option_defaults_and_cancel(recording_backend: RecordingBackend) -> None:
...
def test_sampler_batching_and_mixed_shots(recording_backend: RecordingBackend) -> None:
    ...
    qc.ry(theta, 0)
    pubs = [
        SamplerPub.coerce((qc, {theta: values}), shots=shots) for values, shots in [([[0], [1]], 4), ([2], 8), ([3], 4)]
    ]
    result = backend.sampler().run(pubs).result()
    assert [result[i].data["c"].num_shots for i in range(len(result))] == [4, 8, 4]
    assert [result[i].data["c"].shape for i in range(len(result))] == [(2,), (), ()]
...
def test_estimator_grouping_and_uncertainty(
    recording_backend: RecordingBackend, expected_jobs: int, *, grouping: bool
) -> None:
...
def test_reject_nonpartitioned_classical_bits(recording_backend: RecordingBackend) -> None:
...
def test_estimator_mixed_precision(recording_backend: RecordingBackend) -> None:
    """Native precision groups retain input order and PUB-specific metadata."""
    backend, jobs, _ = recording_backend
    qc = QuantumCircuit(1)
    pubs = [EstimatorPub.coerce((qc, "Z"), precision=precision) for precision in [0.5, 0.25, 0.5]]
    result = backend.estimator().run(pubs).result()
    assert [result[i].metadata["shots"] for i in range(len(result))] == [4, 16, 4]
    assert [job.num_shots for job in jobs] == [4, 4, 16]


def test_estimator_nonzero_uncertainty(recording_backend: RecordingBackend, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backend, "run", run)
    op = SparsePauliOp(["ZI", "IZ", "II"], np.array([2, 3, 4]))  # spellchecker:disable-line
