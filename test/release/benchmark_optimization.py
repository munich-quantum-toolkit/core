...
def main() -> None:
    ...
    if not args.training:
        import numpy as np  # ruff: ignore[import-outside-top-level] Evaluation only; free-threaded training needs no NumPy.
        ...
        circuit = mlir.QCProgram.from_openqasm_str(continuity).to_qiskit()
    ...
    for gates in map(int, args.sizes.split(",")):
        ...
        measure(f"parse/{gates}", lambda: mlir.QCProgram.from_openqasm_str(text), nonempty)
        ...
        measure(f"mapping/{gates}", map_program, nonempty)
