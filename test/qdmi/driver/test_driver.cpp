...
TEST(ConfiguredDriverTest, ConstructionRegistersWithoutOpeningDevices) {
  ...
  EXPECT_NO_THROW(qdmi::Driver::get().registerDevice({.id = "mqt.sc.default",
                                                      .library = library,
                                                      .prefix = prefix,
                                                      .session = {}},
                                                     true));
...
TEST(DeviceRegistrationTest,
  ...
  const auto defaultDevice =
      fomac::Session::openDevice("test.sc.runtime-default");
  const auto customDevice =
      fomac::Session::openDevice("test.sc.runtime-custom");
  ...
  const auto overridden =
      fomac::Session::openDevice("test.sc.runtime-custom", overrides);
  ...
  static_cast<void>(driver.registerDeviceIfAbsent(
      {.id = "test.sc.runtime-invalid",
       ...
       .session = makeDeviceConfigurationSession(
           qdmi::InlineDeviceConfiguration{.json = "{}"})}));
  EXPECT_THROW(
      static_cast<void>(fomac::Session::openDevice("test.sc.runtime-invalid")),
      std::runtime_error);
  EXPECT_EQ(fomac::Session::openDevice("test.sc.runtime-default").getName(),
            "MQT SC Default QDMI Device");
}
