...
TEST_LIFECYCLE_wait_for_finalize(std::size_t timeoutMs);
TEST_LIFECYCLE_QDMI_EXPORT bool
TEST_LIFECYCLE_wait_for_initializations(std::size_t expected,
...
TEST(DeviceApiTest, ReopenWaitsForPriorFinalization) {
  TEST_LIFECYCLE_prepare_blocking_finalize();
  auto first = loadDeviceApi(LIFECYCLE_DEVICE_LIBRARY, "TEST_LIFECYCLE");
  ASSERT_EQ(TEST_LIFECYCLE_initialize_count(), 1);

  std::thread finalize([api = std::move(first)]() mutable { api.reset(); });
  if (!TEST_LIFECYCLE_wait_for_finalize(1000)) {
    TEST_LIFECYCLE_release_finalize();
    finalize.join();
    FAIL() << "device finalization did not start";
  }

  std::barrier reopenReady(2);
  std::shared_ptr<const DeviceApi> reopened;
  std::thread reopen([&] {
    reopenReady.arrive_and_wait();
    reopened = loadDeviceApi(LIFECYCLE_DEVICE_LIBRARY, "TEST_LIFECYCLE");
  });
  reopenReady.arrive_and_wait();

  const auto initializedBeforeFinalize =
      TEST_LIFECYCLE_wait_for_initializations(2, 250);
  TEST_LIFECYCLE_release_finalize();
  finalize.join();
  reopen.join();

  EXPECT_FALSE(initializedBeforeFinalize);
  EXPECT_EQ(TEST_LIFECYCLE_overlapping_initialize_count(), 0);
  EXPECT_EQ(TEST_LIFECYCLE_initialize_count(), 2);
  EXPECT_EQ(TEST_LIFECYCLE_finalize_count(), 1);
  reopened.reset();
  EXPECT_EQ(TEST_LIFECYCLE_finalize_count(), 2);
}
