...
TEST_LIFECYCLE_QDMI_EXPORT bool testLifecycleWaitForFinalize(size_t timeoutMs);
TEST_LIFECYCLE_QDMI_EXPORT bool
testLifecycleWaitForInitializations(size_t expected, size_t timeoutMs);
...
TEST(DeviceApiTest, ReopenWaitsForPriorFinalization) {
  ...
  ASSERT_EQ(testLifecycleInitializeCount(), 1);

  std::thread finalize([api = std::move(first)]() mutable { api.reset(); });
  if (!testLifecycleWaitForFinalize(1000)) {
  ...
  const auto initializedBeforeFinalize =
      testLifecycleWaitForInitializations(2, 250);
  testLifecycleReleaseFinalize();
  finalize.join();
  reopen.join();

  EXPECT_FALSE(initializedBeforeFinalize);
  EXPECT_EQ(testLifecycleOverlappingInitializeCount(), 0);
  EXPECT_EQ(testLifecycleInitializeCount(), 2);
  EXPECT_EQ(testLifecycleFinalizeCount(), 1);
  reopened.reset();
  EXPECT_EQ(testLifecycleFinalizeCount(), 2);
}
