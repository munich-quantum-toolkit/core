...
TEST(QASMSerializer, serializesCompoundOperationBody) {
  const qc::QuantumRegister qreg{0U, 3U, "q"};
  qasm3::QubitIndexToRegisterMap qubitMap{};
  ...
  qasm3::BitIndexToRegisterMap bitMap{};
  ...
  qasm3::Serializer(output3).serialize(compound, qubitMap, bitMap);
  ...
  qasm3::Serializer(output2, qc::Format::OpenQASM2)
...
TEST(QASMSerializer, serializesOnlyContiguousWholeRegisters) {
  const qc::QuantumRegister qreg{0U, 3U, "q"};
  qasm3::QubitIndexToRegisterMap qubitMap{};
  ...
  qasm3::Serializer(output).serialize(compound, qubitMap, bitMap);
...
TEST(QASMSerializer, preservesNegativePeresControlsInOpenQASM2) {
  const qc::QuantumRegister qreg{0U, 3U, "q"};
  qasm3::QubitIndexToRegisterMap qubitMap{};
  qubitMap.try_emplace(0U, qreg, "left");
  qubitMap.try_emplace(1U, qreg, "right");
  ...
  std::ostringstream output{};
  qasm3::Serializer(output, qc::Format::OpenQASM2)
...
TEST(QASMSerializer, indentsNestedIfElseStructure) {
  ...
  std::ostringstream output3{};
  qasm3::Serializer(output3).serialize(operation, qubitMap, bitMap);
...
TEST(QASMSerializer, handlesUnsupportedOperations) {
  const qc::QuantumRegister qreg{0U, 1U, "q"};
  qasm3::QubitIndexToRegisterMap qubitMap{};
  qubitMap.try_emplace(0U, qreg, "q[0]");

  std::ostringstream output{};
  const qasm3::Serializer serializer{output};
...
TEST(QASMSerializer, delegatesCustomOperationsRecursively) {
  const qc::QuantumRegister qreg{0U, 1U, "q"};
  qasm3::QubitIndexToRegisterMap qubitMap{};
  qubitMap.try_emplace(0U, qreg, "q[0]");
  const qc::ClassicalRegister creg{0U, 1U, "c"};
  ...
  std::ostringstream output{};
  const qasm3::Serializer serializer{
      output, qc::Format::OpenQASM2,
      [](std::ostream& customOutput, const qc::Operation& operation,
         const qasm3::QubitIndexToRegisterMap& qubits,
         const qasm3::BitIndexToRegisterMap& bits, const std::size_t indent) {
...
TEST(QASMSerializer, serializesAncillaRegisters) {
