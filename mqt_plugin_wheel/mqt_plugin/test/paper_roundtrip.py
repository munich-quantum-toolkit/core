...
def plugin_roundtrip(num_qubits: int):
    ...
    custom_pipeline = [
        ("before_plugin", ["detensorize-scf", f"test-loop-unrolling{{unroll-factor={num_qubits - 1}}}", "canonicalize"]),
...
def qmap_roundtrip(num_qubits: int):
    ...
    # Transform QASM to MQT core IR
    qc = mqt_load(qasm_str)

    # Benchmark mapping ONLY

    avg_time = timeit.timeit(lambda: qmap_compile(qc, arch), number=NUM_REPS) / NUM_REPS
    return avg_time
...
def plot_results(qasm_data, catalyst_data, plugin_data, qmap_data):
