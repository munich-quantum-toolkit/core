...
- Keep indexed placement, shared if/for/while scalarization, classical constant
  tensor lowering, and explicit tensor-lifetime limits. Current main does not
  replace these fixes.
...
placed tensors hold static references, and unsupported partial dynamic ownership
receives a conversion diagnostic.
...
in DDSIM with `QIR qubit was not dynamically allocated`; the converter now
diagnoses this unsupported lifetime before producing QC.
...
retained compiler contracts. The final ponytail review also removed unused
conversion state and memref exceptions that served only runtime ownership masks.
