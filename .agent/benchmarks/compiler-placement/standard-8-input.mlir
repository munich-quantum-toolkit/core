module {
  func.func @main() -> !cbit.reg<8> attributes {mqt.entry_point} {
    ...
    %alloc = memref.alloc() {mqt.register_name = "query"} : memref<8x!qc.qubit>
    %0 = qc.alloc : !qc.qubit
    %1 = cbit.alloc(#cbit.init<zero>) {mqt.register_name = "result"} : !cbit.reg<8>
    scf.for %arg0 = %c0 to %c8 step %c1 {
      %2 = memref.load %alloc[%arg0] : memref<8x!qc.qubit>
      qc.h %2 : !qc.qubit
    }
    qc.x %0 : !qc.qubit
    scf.for %arg0 = %c0 to %c8 step %c1 {
      %extracted = tensor.extract %cst_1[%arg0] : tensor<8xf64>
      %2 = arith.subi %c7, %arg0 : index
      %3 = memref.load %alloc[%2] : memref<8x!qc.qubit>
    ...
    scf.for %arg0 = %c0 to %c8 step %c1 {
      ...
      %3 = scf.for %arg1 = %c0 to %arg0 step %c1 iter_args(%arg2 = %cst_0) -> (f64) {
        ...
        %6 = memref.load %alloc[%5] : memref<8x!qc.qubit>
        %7 = memref.load %alloc[%arg0] : memref<8x!qc.qubit>
      ...
      %4 = memref.load %alloc[%arg0] : memref<8x!qc.qubit>
      qc.h %4 : !qc.qubit
    }
    scf.for %arg0 = %c0 to %c8 step %c1 {
      %2 = memref.load %alloc[%arg0] : memref<8x!qc.qubit>
      %3 = qc.measure %2 : !qc.qubit -> i1
      cbit.store %3, %1[%arg0] : !cbit.reg<8>
    }
    qc.dealloc %0 : !qc.qubit
    memref.dealloc %alloc : memref<8x!qc.qubit>
    return %1 : !cbit.reg<8>
  }
}
