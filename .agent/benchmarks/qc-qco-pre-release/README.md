...
Production fixes: `37bda5bddf073039f90122d882f6f2eba98a7a38`.
See [the final audit](../../audits/qc-qco-pre-release-2253.md).

![Matched production comparisons](comparison.png)

## Evidence

- `comparison.json`: matched baseline/fixed timings, counts, and output hashes.
- `environment-fixed.json`: exact revisions, source and binary hashes, settings.
- `determinism.json` and `determinism-fixed.json`: 24 scalar and 24 tensor
  modules from separate processes, before and after the fix.
- `tests-fixed.json`: exit status and passing counts for nine rebuilt suites.
- `escaping-index.cpp`, the two `escaping-index-*.mlir` files, and the matching
  logs: the additional correctness finding, reproduced before and after.
- `results.json`, `results.png`, `environment.json`, and `tests.json`: historical
  audit measurements at the upstream baseline. The `noreset` variant only
  disabled the old reset pattern; it was never a proposed production fix.
- `comparison-shared-host.json`: the first complete comparison at the final
  revision, including large scheduling outliers. It is retained separately from
  the repeated complete series plotted above.
...
while retaining existing reset measurements; the final recorded comparison used
one complete run.
...
the final measurements. The host also runs other tasks; the raw spread and the
...
## Reproduce the additional correctness finding
...
Both commands are expected to exit **1**, diagnosing an operand that does not
dominate its use. Their serialized modules are identical. `upstream-index`
compiles the exact baseline builder source and header against the same dialect
libraries; no passes run in this reproducer. `fixed-index` uses the fixed builder.
The reproducer is evidence of an open finding, not a passing regression test.
