...
- [x] (2026-08-11 10:04Z) Publish the signed branch as draft pull request #2054
  without merging it.
- [x] (2026-08-13 21:36Z) Rebase the change onto `origin/main` after the direct
      Qiskit translation merged, resolve the conflicts by retaining the current
      typed-module and Qiskit APIs, and repeat focused C++, Python,
      documentation, stub, and repository lint validation.

## Surprises & Discoveries

- Observation: the translator is one-way despite its broad dependency footprint.
  The only production conversion is `translateQuantumComputationToQC`; there is
  no MLIR-to-`QuantumComputation` exporter. Evidence: the complete repository
...
- Observation: direct Qiskit circuit translation neither constructs a
  `QuantumComputation` nor links `MQT::CoreIR`, so it can remain while the
  legacy translator is removed.
...
- Observation: the Qiskit helper-gate matrix test remains a useful translation
  regression and is retained unchanged.

## Decision Log

- Decision: Remove `QuantumComputation` inputs while retaining direct Qiskit
  circuit inputs. Rationale: direct Qiskit translation imports and exports QC
  MLIR without using the legacy representation, so removing it would add an
  unrelated regression rather than reduce the coupling targeted here.
...
links while retaining direct Qiskit translation and DD functionality. The change
is rebased onto the main branch that contains the direct Qiskit support. Focused
...
Revision note (2026-08-13): rebased after direct Qiskit circuit translation
merged and updated the scope so only the legacy `QuantumComputation` interaction
is removed.
