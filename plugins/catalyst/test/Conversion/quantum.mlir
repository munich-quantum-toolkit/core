...
module {
  ...
  func.func @bar() {
    ...
    // CHECK: %[[H:.*]] = mqtopt.h( static [] mask []) %[[Q2]] : !mqtopt.Qubit
    ...
    %out_h = quantum.custom "Hadamard"() %q2 : !quantum.bit
    ...
    // CHECK: %[[TOF0:.*]], %[[TOFPACKED:.*]]:2 = mqtopt.x( static [] mask []) %[[Z]] ctrl %[[SWAP]]#0, %[[SWAP]]#1 : !mqtopt.Qubit ctrl !mqtopt.Qubit, !mqtopt.Qubit
    ...
    %toffoli:3 = quantum.custom "Toffoli"() %swap#0, %swap#1, %out_z : !quantum.bit, !quantum.bit, !quantum.bit
