...
#include "mlir/Dialect/CC/CCDialect.h"
...
template <typename R> R getParentOfType(Operation* op) {
  do {
    op = op->getParentOp();
...
// gets piggybacked into the canonicalization, but does NOT have anything to do
// with cc::CastOp.
...
OpFoldResult cudaq::cc::ComputePtrOp::fold(FoldAdaptor adaptor) {
  ...
  // substitutions omitted and properly interleaved in as constants in the first
  // list.
...
template <bool quantumAllocs> bool hasAllocation(Region& region) {
...
template <typename A> long countLinearArgs(const A& iterable) {
...
#include "mlir/Dialect/CC/CCOps.cpp.inc"
