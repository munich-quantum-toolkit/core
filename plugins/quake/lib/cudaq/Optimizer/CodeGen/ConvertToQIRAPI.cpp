...
template <bool opaquePtr> Type GetLLVMPointerType(MLIRContext* ctx) {
...
template <bool opaquePtr> struct FullQIR {
  using Self = FullQIR;

  template <typename QuakeOp> static std::string quakeToFuncName(QuakeOp op) {
...
template <bool opaquePtr> struct AnyProfileQIR {
