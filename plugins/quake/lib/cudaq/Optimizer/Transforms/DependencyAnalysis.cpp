...
public:
  ...
  SetVector<DependencyNode*> getNodesAtCycle(unsigned _cycle,
...
private:
  ...
  DenseMap<VirtualQID, DependencyNode*> leaves;
  ...
  void gatherRoots(SetVector<DependencyNode*>& seen, DependencyNode* next) {
    ...
    if (next->isLeaf() && next->isQuantumOp()) {
      leaves.insert({next->qids.front(), next});
  ...
  void replaceLeaf(VirtualQID old_qid, VirtualQID new_qid,
    ...
    if (leaves.count(old_qid) == 1) {
      auto old_leaf = leaves[old_qid];
...
public:
  ...
  OpDependencyNode* getFirstUseOfQID(VirtualQID qid) {
    ...
    DependencyNode* firstUse = leaves[qid]->successors[0];
    if (firstUse->isRoot())
      return nullptr;
  ...
  SetVector<PhysicalQID> getAllocatedQubits() {
    auto allocated = SetVector<PhysicalQID>();
    for (auto [qubit, defining] : qubits)
      if (defining->isAlloc())
        allocated.insert(qubit);
    return allocated;
  }
  ...
  void updateQID(VirtualQID old_qid, VirtualQID new_qid) {
    ...
    auto leaf = leaves[old_qid];
    leaf->updateQID(old_qid, new_qid);

    leaves.erase(leaves.find(old_qid));
    leaves[new_qid] = leaf;
    if (leaf->isAlloc()) {
      allocs.erase(allocs.find(old_qid));
...
public:
  ...
  /// decisions.
  void allocatePhysicalQubits(LifeTimeAnalysis& set) {
...
protected:
  ...
  /// the lifted dealloc, to this `if`. If the physical qubit is present in an
...
public:
  ...
  /// See `DpendencyBlock::allocatePhysicalQubits` for more details on the
...
private:
  ...
  DenseMap<Operation*, SetVector<ShadowDependencyNode*>> freeClassics;
  unsigned vallocs;

public:
  DependencyAnalysisEngine()
      : perOp({}), argMap({}), ifStack({}), freeClassics({}), vallocs(0) {}
  ...
  DependencyNode* visitOp(Operation* op, bool isTerminator) {
    ...
    if (auto init = dyn_cast<quake::BorrowWireOp>(op)) {
      ...
      freeClassics[op] = SetVector<ShadowDependencyNode*>();
      ...
      SetVector<ShadowDependencyNode*> freeIn = freeClassics[op];
      freeClassics.erase(freeClassics.find(op));
  ...
  DependencyNode::DependencyEdge visitValue(Value v) {
    ...
    if (!ifStack.empty() && defOp->getParentOp() != ifStack.back() &&
      ...
      freeClassics[parent].insert(shadow_node);
