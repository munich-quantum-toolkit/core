...
struct Emitter {
  ...
  /// Assigns to `unnamedValue` the name same name of `namedValue`.
  void mapValuesName(mlir::Value namedValue, mlir::Value unnamedValue) {
    assert(valueToName.count(namedValue) && "Unknown named value");
    valueToName.insert(unnamedValue, *valueToName.begin(namedValue));
  }

  void mapValuesName(mlir::ValueRange namedValues,
                     mlir::ValueRange unnamedValues) {
    for (auto [named, unnamed] : llvm::zip(namedValues, unnamedValues)) {
      mapValuesName(named, unnamed);
    }
  }
