...
/// @file Edge.hpp
/// Decision-diagram edges and edge traversal helpers.
...
///
/// This struct is used to represent the core data structure of the DD
...
template <class Node> struct Edge {
  ...
  [[nodiscard]] Result<std::complex<fp>>
  getValueByPath(std::size_t numQubits, const std::string& decisions) const;

  /// Get the size of the DD
  ///
  /// The size of a DD is defined as the number of nodes (including the
...
public:
  ...
  [[nodiscard]] Result<std::complex<fp>> getValueByIndex(std::size_t i) const
    requires IsVector<Node>;
