...
or adjust the code so that no cast is needed. Do not use `module` as a C++
variable or parameter name because it conflicts with the C++20 keyword. Use
`moduleOp` for `mlir::ModuleOp` values.
